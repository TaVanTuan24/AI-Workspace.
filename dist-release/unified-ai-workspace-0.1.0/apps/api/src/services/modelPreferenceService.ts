...
import { OPENAI_COMPAT_MODELS } from "../routes/openaiCompat.js";
...
export async function getModelPreferences(userId: string): Promise<{
  ...
  const views: ModelPreferenceView[] = allModels.map((m, index) => {
    ...
    return {
      ...
      isUsable: health?.isUsable || false,
  ...
  if (!hasDefault && views.length > 0) {
    ...
    const fallback = views.find(v => v.isUsable && v.enabled) || views.find(v => v.enabled) || views[0];
...
export async function resolveDefaultModel(userId: string): Promise<string | null> {
  ...
  if (currentDefault) {
    if (currentDefault.isUsable || !autoSelectFirstUsable) {
