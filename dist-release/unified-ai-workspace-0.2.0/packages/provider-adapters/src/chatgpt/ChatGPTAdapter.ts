...
export class ChatGPTAdapter extends BaseProviderAdapter {
  ...
  async detectLoggedIn(context: BrowserContext): Promise<ProviderAuthStatus> {
    ...
    if (await this.firstVisible(page, CHATGPT_SELECTORS.composerCandidates, 5000)) {
      return "connected";
    }

    if (await this.firstVisible(page, CHATGPT_SELECTORS.loginIndicators, 1000)) {
      return "requires_login";
    }

    if (await this.firstVisible(page, CHATGPT_SELECTORS.manualActionIndicators, 1000)) {
  ...
  async inspectUi(context: BrowserContext): Promise<ProviderUiInspection> {
    ...
    const composerFound = await this.firstVisible(page, CHATGPT_SELECTORS.composerCandidates, 5000);
    const sendButtonFound = await this.firstVisible(page, CHATGPT_SELECTORS.sendButtonCandidates, 1500);
    const responseContainerFound = await this.firstVisible(page, CHATGPT_SELECTORS.responseCandidates, 1500);
    const stopButtonFound = await this.firstVisible(page, CHATGPT_SELECTORS.stopButtonCandidates, 500);
    ...
    return {
      ...
      notes: composerFound ? [] : ["ChatGPT composer was not visible."]
    };
  }
  ...
  async *sendMessage(
    ...
    while (Date.now() - startedAt < RESPONSE_TOTAL_TIMEOUT_MS) {
      ...
      if (await this.firstVisible(page, CHATGPT_SELECTORS.manualActionIndicators, 250)) {
        yield {
      ...
      const stopVisible = await this.firstVisible(page, CHATGPT_SELECTORS.stopButtonCandidates, 250);
      const sendVisible = await this.firstVisible(page, CHATGPT_SELECTORS.sendButtonCandidates, 250);
  ...
  async stopGeneration(context: BrowserContext): Promise<void> {
    const page = await this.firstPage(context);
    for (const selector of CHATGPT_SELECTORS.stopButtonCandidates) {
  ...
  private composerLocator(page: Page) {
    return page.locator(CHATGPT_SELECTORS.composerCandidates.join(", ")).first();
  }
  ...
  private async clickSend(page: Page): Promise<boolean> {
    for (const selector of CHATGPT_SELECTORS.sendButtonCandidates) {
  ...
  private async latestResponseText(page: Page): Promise<string> {
    for (const selector of CHATGPT_SELECTORS.responseCandidates) {
  ...
  private async detectRateLimit(page: Page): Promise<boolean> {
    return this.firstVisible(page, CHATGPT_SELECTORS.rateLimitIndicators, 250);
  }
  ...
  async detectLiveSubModels(context: BrowserContext): Promise<LiveSubModelDetectionResult> {
    ...
    try {
      ...
      // Try to open the model picker if one exists
      // In ChatGPT it's often a button with an aria-haspopup="menu" near the top, or data-testid="model-switcher"
