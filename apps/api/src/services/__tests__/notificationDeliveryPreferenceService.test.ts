...
describe("notificationDeliveryPreferenceService", () => {
  ...
  it("allows enabling a noop channel", async () => {
    const pref = await updateNotificationDeliveryPreference(userId, "test-ws", "email_noop", { enabled: true });
  ...
  it("prevents disabling in_app channel", async () => {
    const pref = await updateNotificationDeliveryPreference(userId, "test-ws", "in_app", { enabled: false });
    // Should stay enabled
    expect(pref.enabled).toBe(true);
