...
export async function providerRoutes(app: FastifyInstance) {
  ...
  app.get("/providers", async (request, reply) => {
    if (!(await requirePermission(request, reply, "providerConnections.read"))) return;
  ...
  app.get("/providers/:provider/connect/status", async (request, reply) => {
    ...
    const requiredPermission = query.connectSessionId ? "providerConnections.write" : "providerConnections.read";
    if (!(await requirePermission(request, reply, requiredPermission))) return;
