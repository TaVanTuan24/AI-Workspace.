...
- **Stream resilience** — initial and conversation-resume navigation retries once on a transient failure (network blip / navigation timeout) before falling back. If the provider session expires mid-response (a login wall appears before any reply text), the turn ends immediately with `requires_login` instead of waiting out the full response timeout.
