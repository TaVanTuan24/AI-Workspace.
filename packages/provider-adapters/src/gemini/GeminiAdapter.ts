    ...
    if (input.attachments && input.attachments.length > 0) {
      ...
      if (!attached) {
        yield {
          ...
          jobId: input.jobId,
          errorCode: "PROVIDER_UI_CHANGED",
          message: "Gemini file attachment input was not found. The provider UI may have changed."
        };
        return;
      }
    }
    ...
    while (Date.now() - startedAt < RESPONSE_TOTAL_TIMEOUT_MS) {
      ...
      if (!sawAnyResponse && polls % 8 === 0 && (await this.detectLoginWall(page))) {
        yield {
          ...
          provider: this.providerId,
          jobId: input.jobId,
          message: "Your Gemini session ended during the response. Please reconnect and try again."
        };
        return;
      }
